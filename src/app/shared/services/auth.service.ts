...
export class AuthService {
  ...
  getCurrentUser() {
    ...
    this._getUser = this.restangular.one('companies', 'me').get().toPromise()
      .then((resp) => {
  ...
  forgot(email: string, type: string): Promise<any> {
    return this.restangular.all('auth/forgot').post({ email, type }).toPromise();
  }
  ...
  companiesRegister(info: any): Promise<any> {
    return this.restangular.all('companies/register').post(info).toPromise();
  }

  companiesLogin(credentials: any): Promise<any> {
    return this.restangular.all('auth/company/login').post(credentials).toPromise()
      .then((resp) => {
        localStorage.setItem('accessToken', resp.data.token);
