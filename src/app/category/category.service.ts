...
export class CategoryService {
  ...
  create(data: any): Promise<any> {
    return this.restangular.all('companies/delivery/categories').post(_.pick(data, this.allowFields)).toPromise();
  }

  tree(): Promise<any> {
    return this.restangular.one('companies/delivery/categories', 'tree').get().toPromise();
  }

  findOne(id): Promise<any> {
    return this.restangular.one('companies/delivery/categories', id).get().toPromise();
  }

  update(id, data): Promise<any> {
    return this.restangular.one('companies/delivery/categories', id).customPUT(_.pick(data, this.allowFields)).toPromise();
  }

  remove(id): Promise<any> {
    return this.restangular.one('companies/delivery/categories', id).customDELETE().toPromise();
  }
